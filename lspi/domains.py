...
class Domain(object):
    ...
    @staticmethod
    def write_edgelist(file_name, graph):
...
class GridMazeDomain(Domain):
    ...
    def __init__(self, height, width, reward_location, walls_location, obstacles_location, initial_state=None,
        ...
        self._reward_fix = reward_location
        self.reward_location = set(self._reward_fix)
    ...
    def generate_unique_samples(self, sample_length, sampling_policy, MAX_TRIALS=20):
        sample = []
        walk = []
        lspi_sample = []
        early_stop = False
        ...
        walk.append(self.current_state()[0])
        visited_state.add(self.current_state()[0])
        for i in range(sample_length):
            s = None
            trials = 0
            while (s is None) and (trials < MAX_TRIALS):
                action = sampling_policy.select_action(self.current_state())
                s = self.apply_action(action, visited_state)
                trials += 1

            if (s is None) or (trials >= MAX_TRIALS):
                return sample, walk, early_stop, lspi_sample

            if s.next_state[0] != s.state[0]:
                sample.append(s)
                walk.append(s.next_state[0])
                visited_state.add(s.next_state[0])
                if not early_stop:
    ...
    def generate_samples(self, sample_length, sampling_policy):
        ...
        for i in range(sample_length):
            ...
            if s.absorb:
                early_stop = True
        actions.append(sampling_policy.select_action(self.current_state()))
        self.reset(self.initial_state)
        # print(lspi_sample)
        # print()
        return sample, walk, actions, early_stop, lspi_sample
    ...
    def apply_action(self, action, visted_states=None):
        ...
        if visted_states is not None and new_location in visted_states:
            # print(visted_states)
            return None
        ...
        if new_location in self.reward_location:
            ...
            # self.reward_location.remove(new_location)
            absorb = True #(len(self.reward_location) == 0)
            sample = Sample(self._state.copy(), action, reward, next_state.copy(), absorb)
            self._state = next_state
            # if absorb:
            #     self.reset(self.initial_state)
        elif new_location in self.obstacles_location:
